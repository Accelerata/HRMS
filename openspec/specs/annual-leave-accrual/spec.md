...
实现年假余额的年度自动发放与入职自动初始化，代替 HR 手动操作。发放规则沿用既有 `calculateAnnualLeaveDays`（首年 0 天、1-9 年 5 天、10-19 年 10 天、20 年+ 15 天），通过定时任务与入职钩子触发，保证幂等。

## Requirements

### Requirement: 年假年度自动发放

系统 SHALL 通过定时任务在每年 1 月 1 日为全体在职员工初始化当年年假余额，年假天数按既有 `calculateAnnualLeaveDays` 计算。发放 MUST 幂等：同一员工同一年重复执行不得产生重复余额行或重复累加（依赖 `uk_employee_leave_year` 唯一约束与 upsert 逻辑）。
